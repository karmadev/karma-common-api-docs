...
| PATCH | `/api/v1/inventory/:id/metadata` | Merge external identifiers or integration context |
| POST | `/api/v1/inventory/commit` | Commit items to an order |

## Mapping external item IDs

Store your SKU, PLU, article number, or other product identifier in the generic `metadata` object. Inventory reads return this object, and the inventory list can filter by its keys.

Use the dedicated metadata PATCH endpoint for a shallow merge so keys owned by another integration are preserved. See [Store External Item IDs](./item-metadata) for create, update, filter, and BigQuery examples.
...
async function pullInventory(
  ...
  while (hasMore) {
    const response = await fetch(
      `https://common-api.karma.life/api/v1/inventory?locationId=${locationId}&includeVariants=true&includeUnavailable=true&page=${page}&limit=100`,
...
async function updateItemAvailability(
  ...
  await fetch(`https://common-api.karma.life/api/v1/inventory/${itemId}`, {
    method: 'PUT',
    headers: {
      'X-API-Key': apiKey,
...
async function bulkUpdateAvailability(
  ...
  for (let i = 0; i < updates.length; i += batchSize) {
    ...
    await Promise.all(
      batch.map(update =>
        fetch(`https://common-api.karma.life/api/v1/inventory/${update.itemId}`, {
          method: 'PUT',
          headers,
...
async function updateItemPrice(
  ...
  await fetch(`https://common-api.karma.life/api/v1/inventory/${itemId}`, {
    ...
    body: JSON.stringify({ basePrice: newPriceCents }), // Always in cents!
  })
...
async function twoWayInventorySync(
  ...
  for (const extItem of externalItems) {
    ...
    if (Object.keys(updates).length > 0) {
      await fetch(`https://common-api.karma.life/api/v1/inventory/${extItem.karmaId}`, {
        method: 'PUT',
        headers,
...
curl -X POST "https://common-api.karma.life/api/v1/webhooks/subscriptions" \
  ...
  -d '{
    ...
    "events": ["inventory.created", "inventory.updated", "inventory.deleted", "inventory.availability_changed"],
...
```bash
curl -X PUT "https://common-api.karma.life/api/v1/variant-groups/:id/options/:optionId" \
  -H "X-API-Key: karma_live_your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{ "available": false }'
```
